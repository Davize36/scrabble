...
const usingBackupDb = ref(true)
const showForfeitNotification = ref(false)
const forfeitMessage = ref('')
...
let lastWriteTime = 0
const WRITE_DELAY = 1000 // 1 second between writes

async function throttledWrite(roomRef, data) {
  const now = Date.now()
  const timeSinceLastWrite = now - lastWriteTime
  
  if (timeSinceLastWrite < WRITE_DELAY) {
    await new Promise(resolve => setTimeout(resolve, WRITE_DELAY - timeSinceLastWrite))
  }
  
  lastWriteTime = Date.now()
  return await throttledWrite(roomRef, data)
}
...
// Computed property for room code color class
const roomCodeClass = computed(() => {
  return usingBackupDb.value ? 'room-highlight-backup' : 'room-highlight'
})
...
function startPingInterval() {
  ...
  pingInterval = setInterval(async () => {
    if (room.value && isJoined.value) {
      const roomRef = doc(currentDb, 'rooms', room.value)
...
async function handleReconnection(roomCode) {
  ...
  const roomRef = doc(currentDb, 'rooms', roomCode)
  try {
...
// FIXED: Proper listener cleanup to prevent read spikes
function startFirebaseListener(roomCode, isReconnect = false) {
  // CRITICAL: Kill ANY existing listener FIRST
  if (unsubscribeSnapshot) {
    console.log("🔥 DESTROYING existing listener before creating new one")
    unsubscribeSnapshot()
  ...
  listenerCount++
  console.log(`📊 Active listeners created: ${listenerCount}`)
  ...
  setTimeout(() => {
    ...
    console.log("✅ Creating NEW single listener for room:", roomCode)
    
    unsubscribeSnapshot = onSnapshot(roomRef, (docSnap) => {
      if (!docSnap.exists()) {
        if (isJoined.value) {
          showForfeitNotification.value = true
          forfeitMessage.value = "The game has ended because a player forfeited."
          setTimeout(() => {
            resetLocalState()
            router.push('/')
          }, 3000)
      ...
      if (data.currentPlayer === -1 && data.latestPlay && data.latestPlay.includes('forfeited')) {
        showForfeitNotification.value = true
        forfeitMessage.value = data.latestPlay
        setTimeout(() => {
          resetLocalState()
          router.push('/')
        }, 3000)
...
async function handleCreateRoom() {
  ...
  // FORCE use backup database directly
  console.log("FORCING backup database...")
  currentDb = db2
  usingBackupDb.value = true
  
  try {
    const roomRef = doc(db2, 'rooms', newRoomCode)
    await setDoc(roomRef, initialData)
    console.log("✅ Room created in BACKUP database!")
    ...
    errorMessage.value = `Failed to create room: ${err.message}`
    isProcessing.value = false
  }
}

async function processRoomJoin(docSnap, code, enteredName, isAutoReconnect) {
  const data = docSnap.data()
  ...
  const roomRef = doc(currentDb, 'rooms', code)
...
async function handleJoinRoom(isAutoReconnect = false) {
  ...
  // Try BACKUP database first
  console.log("Searching for room in BACKUP database...")
  
  try {
    const backupRef = doc(db2, 'rooms', code)
    const backupSnap = await getDoc(backupRef)
    
    if (backupSnap.exists()) {
      console.log("Room found in BACKUP database!")
      currentDb = db2
      usingBackupDb.value = true
      await processRoomJoin(backupSnap, code, enteredName, isAutoReconnect)
      return
    }
  } catch (err) {
    console.log("Backup DB error:", err.message)
  }
  
  // Try PRIMARY database as fallback
  console.log("Searching for room in PRIMARY database...")
  
  try {
    const primaryRef = doc(db, 'rooms', code)
    const primarySnap = await getDoc(primaryRef)
    
    if (primarySnap.exists()) {
      console.log("Room found in PRIMARY database!")
      currentDb = db
      usingBackupDb.value = false
      await processRoomJoin(primarySnap, code, enteredName, isAutoReconnect)
      return
    }
  } catch (err) {
  ...
  errorMessage.value = "Room code not found in any database!"
  isProcessing.value = false
  isReconnecting.value = false
}

// FIXED: Enhanced reset function with proper cleanup
function resetLocalState() {
  console.log("🔄 RESETTING local state and cleaning up...")
  
  // KILL THE LISTENER FIRST
  if (unsubscribeSnapshot) {
    console.log("💀 Killing listener during reset")
    unsubscribeSnapshot()
    unsubscribeSnapshot = null
    isListenerActive = false
    listenerCount--
    console.log(`📊 Active listeners remaining: ${listenerCount}`)
  }
  ...
  isProcessing.value = false
  usingBackupDb.value = true
  showForfeitNotification.value = false
...
async function confirmForfeit() {
  ...
  try {
    ...
    if (opp) {
      const summaryLog = `🏳️ ${name.value} forfeited! ${opp.name} wins the match! 🏆`
    ...
    showForfeitNotification.value = true
    forfeitMessage.value = `You forfeited the match. ${opp?.name || 'Opponent'} wins!`
    ...
    errorMessage.value = "Failed to process forfeit. Please try again."
    isProcessing.value = false
  }
}
...
// FIXED: Enhanced onBeforeUnmount
onBeforeUnmount(() => {
  console.log("🛑 COMPONENT UNMOUNTING - Final cleanup")
  console.log(`📊 Final listener count before cleanup: ${listenerCount}`)
...
<template>
  <!-- Forfeit Notification Popup -->
  <Transition name="fade">
    <div v-if="showForfeitNotification" class="forfeit-notification">
      <div class="forfeit-content">
        <span class="forfeit-icon">🏆</span>
        <h3>Game Over</h3>
        <p>{{ forfeitMessage }}</p>
        <p class="redirect-message">Redirecting to lobby...</p>
      </div>
    </div>
  </Transition>
  ...
  <div v-else class="game container">
    <header class="game-header">
      <div class="header-left-group">
        ...
        <h2>Room Code: <span :class="roomCodeClass">{{ room }}</span></h2>
      </div>
